...
\cvsection{研究专长}
\itemListStart
  \myItem{可扩展 LLM/DNN 训练系统：张量 / 数据 / 流水线 / 专家混合并行策略设计、阶段划分、微批次调度。}
  \myItem{系统协同优化的原理性建模：符号化性能与峰值显存建模、可行性分析、大规模集群验证方法。}
  \myItem{优化式规划算法：以约束规划与整数线性规划联合优化显存、通信与吞吐。}
  \myItem{科研到生产的转化：可复现的基准测试与可部署流程，与工程及平台团队协作落地。}
\itemListEnd
...
\itemListStart
  \myItem{研究课题：使用并行计算技术实现分布式学习的系统化与可移植加速。}
\itemListEnd
\cventry
  {索邦大学}{法国，巴黎}
